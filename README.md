...
本项目二开新增了一个本地 Web 控制台，支持浏览器登录自动保存 Cookie、登录 Cookie 检查、关键词配置、筛选配置、手动采集、自动定时采集和本地文件浏览。
...
- 位置距离：不限
- 默认关键词：男士穿搭
- 默认每个关键词采集 10 篇，点赞 Top N 为 10

采集结果默认会保存到：

```text
datas/markdown_datas/<运行批次>/<关键词>/<笔记标题>.md
datas/markdown_datas/<运行批次>/<关键词>/assert/<笔记标题>/
```

AI 仿写结果默认会单独保存到：

```text
datas/ai_rewrites/<运行批次>/AI仿写_<仿写要求>_<生成批次>/
datas/ai_rewrites/<运行批次>/<关键词>/AI仿写_<仿写要求>_<生成批次>/
```

你也可以在 Web 控制台里直接设置“采集存放目录”。支持绝对路径和相对路径；相对路径会按项目根目录解析。
“AI 仿写存放目录”可以独立设置，文案创作页会在“爬取素材”和“AI仿写”两个目录视图之间切换。

### 🧠 Hermes 全局记忆中心

控制台内置了基于 `NousResearch/hermes-agent` 的后台记忆中心，源码以 Vendor 方式固定在 `vendor/hermes-agent`，当前固定 commit 为 `98d75dea5a86aec599b1e081f8bbe9170bd3f964`。记忆功能默认关闭，需要在“设置 → Hermes 记忆中心”手动开启。

开启后系统会使用 `datas/hermes/` 作为默认 Hermes Home，维护 `MEMORY.md`、`USER.md` 和 SQLite 会话检索库。采集完成、AI 仿写完成、Markdown 改稿保存和创作画像同步时，会写入经过脱敏和摘要处理的长期记忆；Cookie、API Key、登录态和整篇原文不会写入记忆库。

每篇内容直接保存为一个 Markdown 文件，图片、视频和 `info.json` 保存到该关键词主题的 `assert/<笔记标题>/` 文件夹中，Markdown 会加入对应 `info.json` 链接并使用相对路径引用本地媒体文件。
如果同一关键词下出现同名笔记，会自动追加 `-2`、`-3` 这类后缀避免覆盖。
